...
public class ListContestCommand implements ICommand{
    ...
    public void execute(List<String> tokens) {
        try {
            if(tokens.size()==2){
                System.out.println(contestService.getAllContestLevelWise(Level.valueOf(tokens.get(1))).toString().trim());
            }else{
                System.out.println(contestService.getAllContestLevelWise(null).toString().trim());
